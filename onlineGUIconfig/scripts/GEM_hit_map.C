void GEM_hit_map(int layer, TString det = "bb.gem", int what_gep_tracker = 0 /* Input 1 for FT and 2 for FPP */){
  ...
  double height = 2.0;
  double edge = 0.05;
  double vert = 0.176; // Verticle offset for FPP
  ...
  hit2d = new TH2F(Form("hit2d_layer%d",layer),Form("Layer %i;y (m); x (m)",layer),120,-width/2 - edge,width/2 + edge,120,-height/2 - edge,height/2 + edge);
  }
  ...
  else if ( what_gep_tracker == 2 )
  {
    width = 0.6;
    height = 2.0;
  hit2d = new TH2F(Form("hit2d_layer%d",layer),Form("Layer %i;y (m); x (m)",layer),120,-width/2 - edge,width/2 + edge,120,-height/2 - edge - vert,height/2 + edge - vert);
  }
