...
void replay_testlabhist( UInt_t runnum=1234, int nevents=5000, int firstevent=0, UInt_t firstsegment=0, UInt_t maxsegments=1, const char *fname_prefix="vtp_gem_apv_test_3_vtp", Int_t pedestalmode=1, Int_t cmplots=1 )
{	
  ...
  SBSGEMSpectrometerTracker *testgem = new SBSGEMSpectrometerTracker("gemTest", "Test Lab GEM Data"); 

  sbs->AddDetector(testgem);

  //this will override the database setting:
  ( static_cast<SBSGEMTrackerBase *> ( testgem) )->SetPedestalMode( pedestalmode );
  ( static_cast<SBSGEMTrackerBase *> ( testgem ) )->SetMakeCommonModePlots( cmplots );
 ...
 //  if( prefix != "/cache/halla/sbs/GEp/raw" )
  ...
  if( prefix != "/volatile/halla/sbs/adr/testlab_data" )
  pathlist.push_back("/volatile/halla/sbs/adr/testlab_data");
  ...
  while( segcounter < max1 && segment - firstsegment < maxsegments )
  {
	  ...
	  if( ftest == "vtp_gem_apv_test_3_vtp" )
    {
  ...
  if( nevents > 0 ){ 
    outfilebase.Form( "%s_replayed_TestLabGEM_%d_firstevent%d_nevent%d", fname_prefix, runnum, firstevent, nevents );
    outfilename.Form( "%s/%s.root", prefix.Data(), outfilebase.Data());
  }
  else {
    outfilebase.Form( "%s_fullreplay_TestLabGEM_%d", fname_prefix, runnum);
    outfilename.Form( "%s/%s.root", prefix.Data(), outfilebase.Data());
  }
